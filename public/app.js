...
// ntaFullScreenBtn already declared
if (ntaFullScreenBtn) {
    ntaFullScreenBtn.addEventListener('click', () => {
        if (!document.fullscreenElement) {
            document.documentElement.requestFullscreen().catch(err => {
...
// toggle sidebar buttons already declared (some of them)
// already declared
...
function toggleSidebar() {
    ...
    if (isClosed) {
        ...
        if (window.innerWidth < 768) { // Mobile
            ...
            ntaPaletteDrawer.classList.remove('md:translate-x-full');
            ntaPaletteDrawer.classList.add('md:translate-x-0');
            // Shrink the main container by moving it left
        }
    } else {
        ntaPaletteDrawer.classList.add('translate-x-full');
        if (window.innerWidth < 768) {
            ...
            ntaPaletteDrawer.classList.remove('md:translate-x-0');
            ntaPaletteDrawer.classList.add('md:translate-x-full');
        }
    }
}

if (ntaToggleSidebarBtn) ntaToggleSidebarBtn.addEventListener('click', toggleSidebar);
if (ntaMobilePaletteBtn) ntaMobilePaletteBtn.addEventListener('click', toggleSidebar);
if (ntaClosePaletteBtn) ntaClosePaletteBtn.addEventListener('click', toggleSidebar);
if (ntaPaletteOverlay) ntaPaletteOverlay.addEventListener('click', toggleSidebar);

// Desktop starts opened in standard layout. The drawer has classes 'transform translate-x-full md:translate-x-0'.
// Let's fix the toggle logic to correctly work with Tailwind classes.
function toggleNtaSidebarDesktop() {
    ntaPaletteDrawer.classList.toggle('md:translate-x-full');
    ntaPaletteDrawer.classList.toggle('md:hidden');
}
if (ntaToggleSidebarBtn) {
    ntaToggleSidebarBtn.removeEventListener('click', toggleSidebar);
    ntaToggleSidebarBtn.addEventListener('click', toggleNtaSidebarDesktop);
}
